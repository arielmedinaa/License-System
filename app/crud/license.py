...
async def create(db: AsyncSession, *, obj_in: LicenseCreate) -> License:
    ...
    print(obj_in)
    print("PASSWORD RECIBIDO:", repr(obj_in.password))
    print("TIPO:", type(obj_in.password))
    print("LARGO:", len(obj_in.password) if isinstance(obj_in.password, str) else "NO ES STRING")
    ...
    try:
        admin_plain_password = truncate_password(obj_in.password)
        user_data = {
...
async def get_multi(db: AsyncSession, *, skip: int = 0, limit: int = 100) -> List[License]:
    result = await db.execute(select(License).offset(skip).limit(limit))
    return result.scalars().all()


async def update(db: AsyncSession, *, db_obj: License, obj_in: LicenseUpdate) -> License:
